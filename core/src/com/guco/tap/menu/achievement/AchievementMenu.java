...
public class AchievementMenu extends AbstractMenu {
    ...
    public void customizeMenuTable() {
        ...
        claimButton = new TextButton("claim",gameManager.assetsManager.getModuleMenuBuyTxtBtnStyle());
        ...
        for (int i = 0; i<gameManager.achievementManager.achievementElementList.size(); i++){
            Image achievElement = new Image(gameManager.assetsManager.achievementTexture);
        ...
        NinePatch patch = new NinePatch(gameManager.assetsManager.grey9Texture,
                6,6, 6, 6);
        NinePatchDrawable background = new NinePatchDrawable(patch);
        descriptionTable.setBackground(background);

        addMenuHeader("ACHIEVEMENT",1);
    ...
    public void show(){
        for (int i = 0; i<gameManager.achievementManager.achievementElementList.size(); i++) {
            if (gameManager.achievementManager.achievementElementList.get(i).isAchieved || gameManager.achievementManager.achievementElementList.get(i).condition.isAchieved()) {
                ((Image) achievementTable.getCells().get(i).getActor()).setDrawable(new TextureRegionDrawable(gameManager.assetsManager.achievementTexture));
