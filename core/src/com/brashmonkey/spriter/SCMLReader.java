...
public class SCMLReader {
	 ...
	 */
	protected void loadEntities(ArrayList<Element> entities, Element tagList){
		for(int i = 0; i < entities.size(); i++){
			...
			loadAnimations(animations, entity, tagList);
		}
	}
	 ...
	 */
	protected void loadAnimations(ArrayList<Element> animations, Entity entity, Element tagList){
		for(int i = 0; i < animations.size(); i++){
			...
			Element meta = a.getChildByName("meta");
            if (null != meta) {
                Element tagLine = meta.getChildByName("tagline");
                if (null != tagLine) {
                    loadTaglineKeys(tagLine, tagList, animation);
                }
            }
    ...
    /**
     * Iterates through the given taglines and adds them to the given {@link Animation} object.
     */
    protected void loadTaglineKeys(Element tagLine, Element tagList, Animation animation) {

        ArrayList<Element> tagLineKeys = tagLine.getChildrenByName("key");
        ArrayList<Element> varLineKeys = tagLine.getChildrenByName("key");
        ArrayList<Element> tagName = tagList.getChildrenByName("i");

        Meta meta = new Meta(varLineKeys.size(), tagLineKeys.size());

        // iterate throught keys object
        for(int i = 0; i< tagLineKeys.size(); i++) {
            ArrayList<Element> tag = tagLineKeys.get(i).getChildrenByName("tag");
            meta.tagLine.addKey(new Key(tagLineKeys.get(i).getInt("id"), tagLineKeys.get(i).getInt("time",0), tag.size()));
            // Iterate throught tag object
            for(int y = 0; y < tag.size(); y++) {
                meta.tagLine.keys[i].addTag(new Key.Tag(tag.get(y).getInt("id"), tag.get(y).getInt("t",0)));
            }
        }
        loadTagIntoMainline(animation, tagName,  meta.tagLine);
        animation.meta = meta;
    }

    /**
     * Add Tag value to each mainLine key
     * @param animation
     * @param tagName
     * @param tagLine
     */
